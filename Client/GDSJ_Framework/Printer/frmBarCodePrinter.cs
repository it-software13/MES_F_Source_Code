...
namespace GDSJ_Framework.Printer
{
    public partial class frmBarCodePrinter : Form
    {
        ...
        public frmBarCodePrinter(Class.OrgClass Org, string WebServiceUrl, string sqlSelectData)
        {
            ...
            this.Org = Org;
            DB = new GDSJ_Framework.DBHelper.DataBase("SqlServer", Org.DBServer, Org.DBName, Org.DBUser, Org.DBPassword, string.Empty);
        ...
        public frmBarCodePrinter(Class.OrgClass Org, string WebServiceUrl, string sqlSelectData, string printType,DataTable dt)
        {
            InitializeComponent();
            this.dataTable = dt;
            this.sqlSelectData = sqlSelectData;
...
" + where + where1 + @" order by 条码";
        ...
        private string GetBarCode()
        {
            ...
            try
            {
                ...
                string sColumns = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(frm.ReturnDataXML, "<Columns>", "</Columns>");
                List<string> sRows = GDSJ_Framework.Common.StringHelper.GetDataFromTag(frm.ReturnDataXML, "<Row>", "</Row>");

                if (sRows.Count > 0)
                {
                    ColName = sColumns.Split(';')[1].Replace("@", "");
                    ret = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(sRows[0], "<" + sColumns.Split(';')[1].Replace("@", "") + @">", "</" + sColumns.Split(';')[1].Replace("@", "") + @">");
                }
            }
            catch (Exception ex)
            {
