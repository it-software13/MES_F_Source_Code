...
namespace GDSJ_Framework.WinForm.FormDesign.Control
{
        ...
        private void Mi_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (fbc.Text == "确认单据")
                {
                    ...
                    string XML = GDSJ_Framework.Common.WebServiceHelper.RunService(Program.Org, Program.WebServiceUrl, "SJEMS_API", "SJEMS_API.SYS", "DoSure", P2);
                    if (Convert.ToBoolean(Common.StringHelper.GetDataFromFirstTag(XML, "<IsSuccess>", "</IsSuccess>")))
                    {

                        MessageBox.Show("操作成功");
                        GDSJ_Framework.WinForm.FormDesign.FormPanel.HeadPanelClass HPC = this.FC.FormPanels["Panel1"] as GDSJ_Framework.WinForm.FormDesign.FormPanel.HeadPanelClass;
                ...
                if (fbc.Text == "取消确认单据")
                {
                    ...
                    string XML = GDSJ_Framework.Common.WebServiceHelper.RunService(Program.Org, Program.WebServiceUrl, "SJEMS_API", "SJEMS_API.SYS", "NoDoSure", P2);
                    if (Convert.ToBoolean(Common.StringHelper.GetDataFromFirstTag(XML, "<IsSuccess>", "</IsSuccess>")))
                    {
                ...
                if (this.ParentName.IndexOf("HeadPanel") > -1)
                {
                            ...
                            &&MessageBox.Show("是否删除本条数据？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk) == DialogResult.Yes)
                    {
                        ...
                        for (int i = 2; i <= FC.PanelsCount; i++)
                        {
                            ...
                            { 
                            GDSJ_Framework.WinForm.FormDesign.FormPanel.BodyPanelClass BPC =
                               FC.FormPanels["Panel" + i] as GDSJ_Framework.WinForm.FormDesign.FormPanel.BodyPanelClass;
                                GDSJ_Framework.WinForm.FormDesign.Control.DataViewClass DVC =
                                    ((GDSJ_Framework.WinForm.FormDesign.Control.DataViewClass)BPC.Childrens["Children1"]);
