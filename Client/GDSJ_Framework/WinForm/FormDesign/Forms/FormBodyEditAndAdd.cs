...
";
        ...
        private void FormBodyEditAndAdd_FormClosed(object sender, FormClosedEventArgs e)
        {
            ...
            if (this.FPC.PanelType == "HeadPanel")
            {
                ((GDSJ_Framework.WinForm.FormDesign.FormPanel.HeadPanelClass)this.FC.FormPanels["Panel1"]).LoadLastRow();
            }
            else
            {
                ((GDSJ_Framework.WinForm.FormDesign.FormPanel.HeadPanelClass)this.FC.FormPanels["Panel1"]).SetData(this.FC.DataSource.Tables["Table1"].ShowDataRow);
            }
        }
    }
}
