using GDSJ_Framework.WinForm.FormXML.Control;
...
namespace GDSJ_Framework.WinForm.FormXML.Forms
{
    public partial class FormHB : Form
    {
        ...
        public FormHB(string XML, string WebServiceUrl,GDSJ_Framework.Class.OrgClass Org)
        {
            InitializeComponent();
            ...
            this.FC = new GDSJ_Framework.WinForm.FormXML.Forms.FormClass(XML, WebServiceUrl, Org);
        ...
        public FormHB(string XML, string WhereSql, string WebServiceUrl, GDSJ_Framework.Class.OrgClass Org)
        {
            InitializeComponent();
        ...
        private void InitHeadBodyForm(Forms.FormClass fC)
        {
            try
            {
                ...
                ((Panel)hpc.Control).BorderStyle = BorderStyle.FixedSingle;

                panel_HeadMain.Controls.Add((Panel)hpc.Control);
                panel_HeadMain.Height = hpc.Hight + 140;
                ...
                this.panel_BodyMain.Controls.Add(tabBody);
        ...
        private void Tsmi_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DoWorkButtons != null && DoWorkButtons.ContainsKey(mi.Name))
                {
                    fbc = DoWorkButtons[mi.Name];
                }
                if (PrintButtons != null && PrintButtons.ContainsKey(mi.Name))
                {
                    fbc = PrintButtons[mi.Name];
                }
                ...
                if (fbc != null)
                    switch (fbc.Action)
                    {
                        ...
                        case "RunApp":
                            ...
                            GDSJ_Framework.Common.OtherPrograms.RunApp(fbc.DllName, fbc.ClassName, fbc.Method, P);
                            return;
                        case "RunFastReport":
                        ...
                        case "RunService":
                            ...
                            string XML = GDSJ_Framework.Common.WebServiceHelper.RunService(Program.Org, Program.WebServiceUrl, fbc.DllName, fbc.ClassName, fbc.Method, P2);
                        ...
                        case "PrintBarCode":
                            ...
                            System.Data.DataTable dt = GDSJ_Framework.Common.WebServiceHelper.GetDataTable(Program.Org, Program.WebServiceUrl, sql, new Dictionary<string, string>());
                            ...
                            GDSJ_Framework.Printer.frmBarCodePrinter frm = new GDSJ_Framework.Printer.frmBarCodePrinter(Program.Org, Program.WebServiceUrl, dt, fbc.Text);
                            frm.ShowDialog();
                            return;
                        case "PrintBarCode2":
                            string tmp22 = string.Empty;
                            string tmp3 = string.Empty;
                            foreach (string s in fbc.Parameters.Keys)
                            {
                                if (fbc.Parameters[s].StartsWith("HeadData."))
                                {
                                    tmp22 = FC.DataSource.Tables["Table1"].ShowDataRow[fbc.Parameters[s].Replace("HeadData.", "")].ToString();
                                    tmp3 = "@" + fbc.Parameters[s].Replace("HeadData.", "");
                                }
                            ...
                            dt = GDSJ_Framework.Common.WebServiceHelper.GetDataTable(Program.Org, Program.WebServiceUrl, sql, new Dictionary<string, string>());
                            ...
                            frm = new GDSJ_Framework.Printer.frmBarCodePrinter(Program.Org, Program.WebServiceUrl, sql1, fbc.Text, dt);
                            frm.ShowDialog();
                            return;
        ...
        public void SetStatus(string Status)
        {
            ...
            this.Status = Status;
                ...
                FormPanel.BodyPanelClass bpc = (FormPanel.BodyPanelClass)this.FC.FormPanels["Panel" + (tabBody.SelectedIndex + 2)];

                
                    bpc.SetStatus(Status);
            ...
            if (e.KeyChar == 13)
            {
                if (!string.IsNullOrEmpty(t.Text.Trim()))
                {
                    ...
                    if (sRows.Count > 0)
                    {
                        ...
                        string num = (this.tabBody.SelectedIndex + 2).ToString();

                        ((FormPanel.BodyPanelClass)this.FC.FormPanels["Panel" + num]).SetHeadData(dr);
        ...
        private void B_Click(object sender, EventArgs e)
        {
            ...
            if (((ToolStripButton)sender).Name == "btn_Back" && Table.DataRow != null)
            {
                ...
                ((FormPanel.BodyPanelClass)this.FC.FormPanels["Panel" + (this.tabBody.SelectedIndex + 2)]).SetHeadData(dr);
            ...
            else if (((ToolStripButton)sender).Name == "btn_Last")
            {
                ...
                ((FormPanel.BodyPanelClass)this.FC.FormPanels["Panel" + (this.tabBody.SelectedIndex + 2)]).SetHeadData(hpc.Data);
            ...
            {
                DataViewClass dvc = FC.FormPanels["Panel" + (this.tabBody.SelectedIndex + 2)].Childrens["Children1"] as DataViewClass;
                if (dvc.checkRowData())
                {
...
";
                            ...
                            if (Table.AddRow(FC.Org, Program.WebServiceUrl))
                            {
                                ...
                                ((FormPanel.BodyPanelClass)this.FC.FormPanels["Panel" + num]).SetHeadData(Table.ShowDataRow);
                        ...
                        && MessageBox.Show("是否删除本条数据？", "提示", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk) == DialogResult.Yes)
                {
                    if (FC.PanelsCount > 1)
                        for (int i = 2; i <= FC.PanelsCount; i++)
                        {
                            ...
                            {
                                GDSJ_Framework.WinForm.FormXML.FormPanel.BodyPanelClass BPC =
                                   FC.FormPanels["Panel" + i] as GDSJ_Framework.WinForm.FormXML.FormPanel.BodyPanelClass;
                                GDSJ_Framework.WinForm.FormXML.Control.DataViewClass DVC =
                                    ((GDSJ_Framework.WinForm.FormXML.Control.DataViewClass)BPC.Childrens["Children1"]);
               ...
               !string.IsNullOrEmpty(Table.ShowDataRow["dosureby"].ToString()))
                {
                    if (MessageBox.Show("是否需要反确认数据？", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        ...
                        XML = GDSJ_Framework.Common.WebServiceHelper.RunService(Program.Org, Program.WebServiceUrl, "SJEMS_API", "SJEMS_API.SYS", "NoDoSure", P);
                        if (Convert.ToBoolean(Common.StringHelper.GetDataFromFirstTag(XML, "<IsSuccess>", "</IsSuccess>")))
                        {
               ...
               string.IsNullOrEmpty(Table.ShowDataRow["dosureby"].ToString()))
                {
                    ...
                    if (MessageBox.Show("是否需要确认数据？", "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        ...
                        XML = GDSJ_Framework.Common.WebServiceHelper.RunService(Program.Org, Program.WebServiceUrl, "SJEMS_API", "SJEMS_API.SYS", "DoSure", P);
                        if (Convert.ToBoolean(Common.StringHelper.GetDataFromFirstTag(XML, "<IsSuccess>", "</IsSuccess>")))
                        {
        ...
        private void SaveBody()
        {
            ...
            TableClass Table = this.FC.DataSource.Tables["Table" + (tabBody.SelectedIndex + 2)];

            DataViewClass dvc = bpc.Childrens["Children1"] as DataViewClass;

            DataGridView dgv = ((DataGridView)dvc.Control.Controls.Find(dvc.ParentName + "_DataView", false)[0]);
            ...
            foreach(DataRow dr in dvc.ShowData.Rows)
            {
                if (dr["id"].ToString()=="-1" || string.IsNullOrEmpty(dr["id"].ToString()))
                {
                    string sql2 = string.Empty;
                    string IC = string.Empty;
                    string IV = string.Empty;
                    sql2 = @"INSERT INTO [" + Table.TableName + @"] (";

                    foreach (string key in dvc.Columns.Keys)
                    {

                        IC += dvc.Columns[key].DataKey + ",";

                        if (dvc.Columns[key].DataKey == "org")
                        {
                            IV += "'" + FC.Org.Org + "',";
                        }
                        else if (dvc.Columns[key].DataKey == "createby")
                        {
                            IV += "'" + FC.Org.User.UserCode + "',";

                        }
                        else if (dvc.Columns[key].DataKey == "createdate")
                        {
                            IV += "'" + DateTime.Now.ToString("yyyy-MM-dd") + "',";

                        }
                        else if (dvc.Columns[key].DataKey == "createtime")
                        {
                            IV += "'" + DateTime.Now.ToString("HH:mm:ss") + "',";

                        }
                        else
                        {

                            IV += "'" + dr[dvc.Columns[key].DataKey].ToString() + "',";
...
";
                ...
                else
                {
                    ...
                    sql2 = @"UPDATE [" + Table.TableName + @"] SET ";

                    foreach (string key in dvc.Columns.Keys)
                    {
                        if (dvc.Columns[key].DataKey == "org")
                        {
                            sql2 += dvc.Columns[key].DataKey + "='" + FC.Org.Org + "',";
                        }
                        else if (dvc.Columns[key].DataKey == "modifyby")
                        {
                            sql2 += dvc.Columns[key].DataKey + "='" + FC.Org.User.UserCode + "',";

                        }
                        else if (dvc.Columns[key].DataKey == "modifydate")
                        {
                            sql2 += dvc.Columns[key].DataKey + "='" + DateTime.Now.ToString("yyyy-MM-dd") + "',";

                        }
                        else if (dvc.Columns[key].DataKey == "modifytime")
                        {
                            sql2 += dvc.Columns[key].DataKey + "='" + DateTime.Now.ToString("HH:mm:ss") + "',";

                        }
                        else
                        {
                            sql2 += dvc.Columns[key].DataKey + "='" + dr[dvc.Columns[key].DataKey].ToString() + "',";
                        }
                    }

                    sql2 = sql2.Remove(sql2.Length - 1)+@" WHERE id="+dr["id"].ToString()+@"
";
            ...
            GDSJ_Framework.Common.WebServiceHelper.ExecuteNonQuery(FC.Org, Program.WebServiceUrl, sql, new Dictionary<string, string>());

            sql = string.Empty;
            
            foreach(int id in dvc.DelId)
            {
                sql+= @"
