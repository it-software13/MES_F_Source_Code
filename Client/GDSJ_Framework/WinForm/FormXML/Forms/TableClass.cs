...
";
        ...
        public System.Data.DataTable GetDataTable(string WebServiceUrl, string strWhere)
        {
...
";
        ...
        public System.Data.DataTable GetDataTable(Class.OrgClass Org, string WebServiceUrl, string strWhere)
        {
...
";
        ...
        public System.Data.DataTable GetShowDataTable(Class.OrgClass Org, string WebServiceUrl, string sql)
        {
...
";
            for(int i=0;i<OrderKeys.Count;i++)
            {
                if(string.IsNullOrEmpty(order))
                {
                    ...
                    {
                        order += " ORDER BY [" + 
                        GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(OrderKeys[i], "[", "]") + "] " + OrderTypes[i] + " ";
                    }
                    else
                    {
                        order += " ORDER BY [" +
...
";
            for (int i = 0; i < OrderKeys.Count; i++)
            {
                ...
                else
                {
                    if (OrderKeys[i].IndexOf("[") > -1 && OrderKeys[i].IndexOf("]") > -1)
                    {
                        order += ", [" + GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(OrderKeys[i], "[", "]") + "] " + OrderTypes[i] + " ";
                    }
                    else
                    {
                        order += ", [" + OrderKeys[i] + "] " + OrderTypes[i] + " ";
        ...
        public System.Data.DataRow GetNewRow(GDSJ_Framework.Class.OrgClass Org, string WebServiceUrl)
        {
