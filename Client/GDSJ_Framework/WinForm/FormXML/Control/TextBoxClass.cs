...
namespace GDSJ_Framework.WinForm.FormXML.Control
{
    public class TextBoxClass: ControlClass
    {
        ...
        private void TxtValue_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            if ((this.Status == "Add" && this.Add) || ( this.Status == "Edit" && this.Edit))
            {
                ...
                {
                    List<string> ss = GDSJ_Framework.Common.StringHelper.GetDataFromTag(sql, "@DATA.", "@");

                    FormPanel.HeadPanelClass hpc = this.FC.FormPanels["Panel1"] as FormPanel.HeadPanelClass;
                    ...
                    foreach (string s in ss)
                    {
                        ...
                        {
                            TextBoxClass tbc = (TextBoxClass)hpc.Childrens[key];
