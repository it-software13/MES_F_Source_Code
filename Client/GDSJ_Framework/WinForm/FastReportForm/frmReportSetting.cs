...
namespace GDSJ_Framework.WinForm.FastReportForm
{
    public partial class frmReportSetting : Form
    {
        ...
        private void LoadDataSourceName()
        {
            ...
            string ReportStr = GDSJ_Framework.Common.TXTHelper.ReadToEnd(txt_ReportPath.Text);
            string dictionary = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(ReportStr, "<Dictionary>", "</Dictionary>");
            var lstDataSource = GDSJ_Framework.Common.StringHelper.GetDataFromTag(dictionary, "<TableDataSource", "</TableDataSource>");
        ...
        private void LoadData()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                string DataSources = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(dt.Rows[0]["ReportXML"].ToString(), "<DataSources>", "</DataSources>");
                txt_Primarykey.Text = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(DataSources, "<DataKey>", "</DataKey>");
                comboBox1.Text = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(DataSources, "<IsShow>", "</IsShow>");

                var lstDataSource = GDSJ_Framework.Common.StringHelper.GetDataFromTag(DataSources, "<DataSource>", "</DataSource>");
                TabControl tab = new TabControl();
                tab.SelectedIndexChanged += Tab_SelectedIndexChanged;
                tab.Dock = DockStyle.Fill;
                foreach (var item in lstDataSource)
                {
                    ...
                    TabPage page = new TabPage();
                    page.Text = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(item, "<TableName>", "</TableName>");
                    ...
                    txt.Multiline = true;
                    txt.ScrollBars = ScrollBars.Both;
                    txt.Text = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(item, "<SearchSQL>", "</SearchSQL>");
                    txt.Dock = DockStyle.Fill;
                    panel.Controls.Add(txt);
                    page.Controls.Add(panel);
                ...
                string ReportQueryBox = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(dt.Rows[0]["ReportXML"].ToString(), "<ReportQueryBox>", "</ReportQueryBox>");
                var lstQueryBox = GDSJ_Framework.Common.StringHelper.GetDataFromTag(ReportQueryBox, "<QueryBox>", "</QueryBox>");
                panelCtr = new Panel();
                panelCtr.Name = "QueryBox";
                panelCtr.Padding = new Padding(10, 10, 10, 10);
                panelCtr.Location = new Point(0, 45);
                ...
                foreach (var item in lstQueryBox)
                {
                    ...
                    if (GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(item, "<DataType>", "</DataType>").ToLower() == "string")
                    {
                        TextBox txt = new TextBox();
                        ...
                        txt.Text = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(item, "<Title>", "</Title>");
                        txt.Name = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(item, "<Title>", "</Title>");
                        txt.Tag = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(item, "<DataSelectSQL>", "</DataSelectSQL>");
                        panelCtr.Controls.Add(txt);
                    }
                    else if(GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(item, "<DataType>", "</DataType>").ToLower() == "date")
                    {
                        ...
                        dtp.Name = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(item, "<Title>", "</Title>");
                    ...
                    }else
                    {
                        ...
                        cbo.Text = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(item, "<Title>", "</Title>");
                        cbo.Name = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(item, "<Title>", "</Title>");
                        cbo.Tag = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(item, "<DataSelectSQL>", "</DataSelectSQL>");
                                            ...
                                            foreach (Control t in p.Controls)
                                            {
                                                ...
                                                if (t is TextBox)
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        pp.Add("sql", t.Text);
                                                        string XML = GDSJ_Framework.Common.WebServiceHelper.RunService(Program.Org, Program.WebServiceUrl, "SJEMS_API", "SJEMS_API.DataBase", "GetDataTable", pp);
                                                        string dtXML = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(XML, "<RetData>", "</RetData>");

                                                        tabData = GDSJ_Framework.Common.StringHelper.GetDataTableFromXML(dtXML);
                                                    }
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(reportCode))
                {
                    ...
                    }else
                    {
                        Dictionary<string, string> pp = new Dictionary<string, string>();
                        pp.Add("sql", sql);
                        string XML = GDSJ_Framework.Common.WebServiceHelper.RunService(Program.WebServiceUrl, "SJEMS_API", "SJEMS_API.DataBase", "GetDataTable", pp);
                        string dtXML = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(XML, "<RetData>", "</RetData>");
                        tabData = GDSJ_Framework.Common.StringHelper.GetDataTableFromXML(dtXML);
                    }
                    ...
                    }else
                    {
                        Dictionary<string, string> pp = new Dictionary<string, string>();
                        pp.Add("sql", sql);
                        string XML = GDSJ_Framework.Common.WebServiceHelper.RunService(Program.WebServiceUrl, "SJEMS_API", "SJEMS_API.DataBase", "ExecuteNonQuery", pp);
                        b = Convert.ToBoolean(GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(XML, "<IsSuccess>", "</IsSuccess>"));
                    }
                    if (b)
                    {
                        isTrue = true;
        ...
        private void btn_Pre_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    string XML = GDSJ_Framework.Common.WebServiceHelper.RunService(Program.WebServiceUrl, "SJEMS_API", "SJEMS_API.DataBase", "GetDataTable", pp);
                    string dtXML = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(XML, "<RetData>", "</RetData>");
                    tab = GDSJ_Framework.Common.StringHelper.GetDataTableFromXML(dtXML);
            ...
            GDSJ_Framework.WinForm.FastReportForm.frmFastReport frm = new GDSJ_Framework.WinForm.FastReportForm.frmFastReport(reportPath, xml, connectionString, reportString);
            frm.Show();
        }
