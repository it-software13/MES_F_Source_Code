        ...
        public FrmReportMain()
        {
            ...
            if (Convert.ToInt32(tab.Rows[0][0]) == 0)
            {
               string sql = "use SJEMSSYS CREATE TABLE SYSREPORT01M(id int identity(1,1) NOT NULL,ReportCode  varchar(200) not null,ReportName varchar(500) not null,ReportPath nvarchar(500) not null,ReportXML nvarchar(max),ReportString nvarchar(max),ModuleNo nvarchar(200))";
                ...
                p.Add("sql", sql);
                XML = GDSJ_Framework.Common.WebServiceHelper.RunService(Program.WebServiceUrl, "SJEMS_API", "SJEMS_API.DataBase", "ExecuteNonQuery", p);
                // GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(XML, "<IsSuccess>", "</IsSuccess>");
            }
...
where tab.序号 > " + (pageNo - 1) * 30;
                    p = new Dictionary<string, string>();
                    p.Add("sql", sql);
                    XML = GDSJ_Framework.Common.WebServiceHelper.RunService(Program.WebServiceUrl, "SJEMS_API", "SJEMS_API.DataBase", "GetDataTable", p);
                    dtXML = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(XML, "<RetData>", "</RetData>");
                    tab = GDSJ_Framework.Common.StringHelper.GetDataTableFromXML(dtXML);
                }
        ...
        private void btn_Delete_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("确定要删除报表数据？", "删除报表", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                ...
                }else
                {
                    Dictionary<string, string> p = new Dictionary<string, string>();
                    p.Add("sql", sql);
                    string XML = GDSJ_Framework.Common.WebServiceHelper.RunService(Program.WebServiceUrl, "SJEMS_API", "SJEMS_API.DataBase", "ExecuteNonQuery", p);
        ...
        private void btn_DownLoad_Click(object sender, EventArgs e)
        {
            ...
            if (fbd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                if (b)
                {
                    ...
                    if (GDSJ_Framework.Common.TXTHelper.WriteToEnd(fbd.SelectedPath + "\\" + tab.Rows[0]["ReportName"].ToString() + ".frx", tab.Rows[0]["ReportString"].ToString()))
        ...
        private void btn_preView_Click(object sender, EventArgs e)
        {
            ...
            {
                GDSJ_Framework.WinForm.FastReportForm.frmFastReport frm = new GDSJ_Framework.WinForm.FastReportForm.frmFastReport(tab.Rows[0]["ReportPath"].ToString(), tab.Rows[0]["ReportXML"].ToString().Replace("@"+ HeadDataKey,DocNo), connectionString, tab.Rows[0]["ReportString"].ToString());
                frm.Show();
            }else
            {
