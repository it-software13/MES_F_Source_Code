    ...
    public partial class frmFastReport : Form
    {
        ...
        private void frmFastReport_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                string title = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(fastReportXml, "<Title>", "</Title>");
                this.Text = title;
                ...
                #region 查询文本框
                string reportQueryBox = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(fastReportXml, "<ReportQueryBox>", "</ReportQueryBox>");
                List<string> lstQueryBox = GDSJ_Framework.Common.StringHelper.GetDataFromTag(reportQueryBox, "<QueryBox>", "</QueryBox>");
                ...
                foreach (var item in lstQueryBox)
                {
                    ...
                    Label lbl = new Label();
                    lbl.Text = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(item, "<Title>", "</Title>");
                    lbl.Location = new Point(lblX, lblY);
                    lbl.Size = new Size(53, 12);
                    panel.Controls.Add(lbl);

                    string dataType = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(item, "<DataType>", "</DataType>");
                    string dataSelectSQL = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(item, "<DataSelectSQL>", "</DataSelectSQL>");
                ...
                #region 设置数据源
                string dataSources = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(fastReportXml, "<DataSources>", "</DataSources>");

                string dataKey = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(dataSources, "<DataKey>", "</DataKey>");//关联字段

                bool isShow = Convert.ToBoolean(GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(dataSources, "<IsShow>", "</IsShow>"));//是否显示数据

                List<string> lstDataSources = GDSJ_Framework.Common.StringHelper.GetDataFromTag(dataSources, "<DataSource>", "</DataSource>");
     
                    if (isShow)
                    {
                        foreach (var item in lstDataSources)
                        {
                            string tableName = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(item, "<TableName>", "</TableName>");
                            string searchSQL = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(item, "<SearchSQL>", "</SearchSQL>");
                                ...
                                foreach (var item in lstDataSources)
                                {
                                    ...
                                    if (n == 1)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            string XML = GDSJ_Framework.Common.WebServiceHelper.RunService(Program.Org, Program.WebServiceUrl, "SJEMS_API", "SJEMS_API.DataBase", "GetDataTable", p);
                                            string dtXML = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(XML, "<RetData>", "</RetData>");
        ...
        private void Txt_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (DB != null)
                {
                    ...
                    {
                        txt.Text = GDSJ_Framework.Common.StringHelper.GetDataFromFirstTag(frm.ReturnDataXML, "<" + txt.Name + ">", "</" + txt.Name + ">");
                    }
                }
                else
                {
