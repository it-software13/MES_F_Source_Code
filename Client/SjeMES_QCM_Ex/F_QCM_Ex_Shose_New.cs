...
{
    public partial class F_QCM_Ex_Shose_New : MaterialForm
    {
        ...
        private void dgv_cpx_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.RowIndex > -1 && !ckb_cpx_sfcc.Checked)
            {
                if (dgv_cpx.Columns[e.ColumnIndex].Name == "cpx_tygs")
                {
                    ...
                    {
                        CB_G_formula_type.SelectedIndex = 0;//1
        ...
        private void btn_bj_sure_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    MessageBoxButtons messButton = MessageBoxButtons.OKCancel;
                    DialogResult dr = MessageBox.Show("This ART has been submitted for component testing and registration, do you want to continue?", "Prompt", messButton);
        ...
        private void btn_gy_sure_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    MessageBox.Show("Please enter or select a manufacturer/production line");
                    txt_gy_cs.Focus();
                    return;
                }
                else
        ...
        public void Get_cl_checkItem(object sender, EventArgs e)
        {
            ...
            data.Add("fgt_code", cmb_cl_fgt.SelectedIndex > 0 ? cmb_cl_fgt.SelectedValue : "");
            //键值对传值
            string retdata = SJeMES_Framework.WebAPI.WebAPIHelper.Post(
                                        Program.Client.APIURL,
                                        "SJ_QCMAPI",//类库名
        ...
        string CL_QRCODE_JSON = "";
        string save_cl_qrcode = "";
        string part_list_str = "";//材料部件
        string color_str = "";//材料颜色
        private void btn_cl_sure_Click(object sender, EventArgs e)
        {
            ...
            {
                if (string.IsNullOrEmpty(txt_cl_cc_task_no.Text.Trim()))
                {
                    MessageBox.Show("Please scan the retest lab number");
            ...
            else
            {
                ...
                if (string.IsNullOrEmpty(save_cl_qrcode))
                {
                    MessageBox.Show("Please enter or scan the QR code of the receipt");
                    txt_cl_cs.Focus();
                    return;
                }

                if (string.IsNullOrEmpty(CL_QRCODE_JSON))
                {
                    MessageBox.Show("Wrong QR code of receipt");
                    txt_cl_cs.Focus();
                    return;
                }
                ...
                data.Add("cl_code", save_cl_qrcode);
                ...
                data.Add("test_id", txt_cl_test_id.Text.Trim());
                data.Add("itemlist", itemlist);
                data.Add("sldh", save_cl_qrcode);
                data.Add("cl_qrcode_json", CL_QRCODE_JSON);//材料收料单二维码json信息
                data.Add("makings_id", tb_cl_lh.Text);//料号
                data.Add("send_test_qty", "1");//送测数量
                string art_no = "";
                string shoe_no = "";
                string artandshoe_str = tb_artandshoe.Text.ToString().Trim();
                if (!string.IsNullOrEmpty(artandshoe_str))
                {
                    var artandshoe_list = artandshoe_str.Split(',').ToList();
                    List<string> art_no_list = new List<string>();
                    List<string> shoe_no_list = new List<string>();
                    foreach (var item in artandshoe_list)
                    {
                        if (!string.IsNullOrEmpty(item))
                        {
                            var artandshoe_info = item.Split('/');
                            art_no_list.Add(artandshoe_info[1]);
                            shoe_no_list.Add(artandshoe_info[0]);
                        }
                    }
                    art_no = string.Join(",", art_no_list);
                    shoe_no = string.Join(",", shoe_no_list);
                }
                string order_po = "";
                string order_po_qty = "";
                string order_po_str = tb_order_number.Text.ToString().Trim();
                if (!string.IsNullOrEmpty(order_po_str))
                {
                    var order_po_list = order_po_str.Split(',').ToList();
                    List<string> order_po_arr = new List<string>();
                    List<string> order_po_qty_arr = new List<string>();
                    foreach (var item in order_po_list)
                    {
                        if (!string.IsNullOrEmpty(item))
                        {
                            var oder_po_info = item.Split('/');
                            order_po_arr.Add(oder_po_info[0]);
                            order_po_qty_arr.Add(oder_po_info[1]);
                        }
                    }
                    order_po = string.Join("/", order_po_arr);
                    order_po_qty = string.Join("/", order_po_qty_arr);
                }
                data.Add("order_po", order_po);//订单po
                data.Add("order_po_qty", order_po_qty);//po数量
                data.Add("art_no", art_no);//art
                data.Add("shoe_no", shoe_no);//鞋型
                data.Add("position_code", part_list_str);//部位
                data.Add("position_name", part_list_str);//部位
                data.Add("colors", color_str);//color
        ...
        private void btn_lcll_sure_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                // If else condition Added by Ashok to avoid duplicate entry on 2025/07/21
                if (Check_Duplicate())
                {
                    SJeMES_Control_Library.MessageHelper.ShowErr(this, "This PO is already entered in this Line");
                    return;
                }
                else
                {
                ...
                if (Convert.ToInt32(ret1.RetData) > 0)
                {
                    ...
                    data.Add("category_name", txt_lcll_category.Text.Trim());
        ...
        private void ckb_cl_sfcc_CheckedChanged(object sender, EventArgs e)
        {
            ...
            tb_cl_lh.Text = "";
            tb_cl_clmc.Text = "";
            tb_cl_suoyongbuweimingcheng.Text = "";
            tb_artandshoe.Text = "";
            tb_order_number.Text = "";
        }
        ...
        private void txt_cl_cc_task_no_KeyUp(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                BindDDL(cmb_cl_fgt, list_fgt_data, "cl", true, true);
                //请求api的数据展示
                Dictionary<string, object> p = new Dictionary<string, object>();
                p.Add("task_no", txt_cl_cc_task_no.Text.Trim());
                p.Add("test_type", "3");// chenged
                string retdata = SJeMES_Framework.WebAPI.WebAPIHelper.Post(
                                            Program.Client.APIURL,
                                            "SJ_QCMAPI",//类库名
                                            "SJ_QCMAPI.ExShose",//类名
                ...
                save_cl_qrcode = info["ART_NO"].ToString();
                ...
                CL_QRCODE_JSON = info["CL_QRCODE_JSON"].ToString();
        ...
        private void txt_lcll_cc_task_no_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                try
                {
                    ...
                    //txt_cl_qrcode.Text = "{\"rcpt_date\":\"2022-10-06\",\"item_no\":\"4020201303\",\"chk_seq\":\"10\",\"chk_no\":\"SL100011111185\",\"org_id\":\"1001\"}";
                    BindDDL(cmb_cl_fgt, list_fgt_data, "cl", true, true);
                    ...
                    if (dic.ContainsKey("rcpt_date") && dic.ContainsKey("item_no") && dic.ContainsKey("chk_seq") && dic.ContainsKey("chk_no") && dic.ContainsKey("org_id"))
                    {
                        txt_chk_no(dic["rcpt_date"].ToString(), dic["item_no"].ToString(), dic["org_id"].ToString(), dic["chk_no"].ToString());
                    }
                    else
                    {
                        //string msg = SJeMES_Framework.Common.UIHelper.UImsg("无效二维码", Program.Client, Program.Client.WebServiceUrl, Program.Client.Language);
                        //SJeMES_Control_Library.MessageHelper.ShowErr(this, msg);
                        //MessageBox.Show("无效二维码");
                        txt_cl_qrcode.Text = "";
                        save_cl_qrcode = "";
                        part_list_str = "";
                        color_str = "";
        ...
        private void txt_chk_no(string rcpt_date,string item_no,string org_id,string chk_no)
        {
            ...
            {
                GetQrCodeInfo(txt_cl_qrcode.Text.Trim());

                CL_QRCODE_JSON = txt_cl_qrcode.Text.Trim();

                Dictionary<string, string> ret_res = Newtonsoft.Json.JsonConvert.DeserializeObject<Dictionary<string, string>>(ret.RetData);
                txt_cl_qrcode.Text = ret_res["chk_nolist"];
                save_cl_qrcode = ret_res["chk_nolist"];
                part_list_str = ret_res["part_list_str"];
                color_str = ret_res["color_str"];
            }
        }
        ...
        public void GetQrCodeInfo(string jsonKey, bool suppliers_read = true)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (suppliers_read)
                {
                    ...
                    lab_cl_cs_jc.Text = fRow["JC"].ToString();
                }
        ...
        private void txt_cpx_ddpo_TextChanged(object sender, EventArgs e)
        {

        }

        private void txt_cl_cs_TextChanged(object sender, EventArgs e)
        {
