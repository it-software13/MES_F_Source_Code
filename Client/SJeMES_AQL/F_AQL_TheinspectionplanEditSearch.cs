...
{
    public partial class F_AQL_TheinspectionplanEditSearch : MaterialForm
    {
        ...
        public F_AQL_TheinspectionplanEditSearch(string _id,string _plan_date,string _level_type)
        {
            InitializeComponent();
            id = _id;//表头Id
...
Program.SkinThemes, materialSkinManager, this);
        ...
        public F_AQL_TheinspectionplanEditSearch()
        {
            InitializeComponent();
            materialSkinManager = SJeMES_Control_Library.MaterialSkin.MaterialSkinHelper.MaterialSkinManagerSetDefault(
Program.SkinThemes, materialSkinManager, this);
        ...
        public void GetDataList(int pageSize, int pageIndex, out int totalCount)
        {
            ...
            try
            {
                ...
                string gongchangStr = "";
                if (!string.IsNullOrWhiteSpace(txt_gongchang.Text) && !string.IsNullOrWhiteSpace(ORG_ID))
                {
                    gongchangStr = ORG_ID;
                }
                p.Add("zubie", txt_zubie.Text.Trim());
                p.Add("gongchang", gongchangStr);
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        dgvr.Cells["工厂"].Value = dr["工厂"].ToString();//存放区域
...
";
            ...
            {
                txt_gongchang.Text = frmData.RetData.Rows[0]["Warehouse_Name"].ToString();
                ORG_ID = frmData.RetData.Rows[0]["Warehouse_Code"].ToString();
                ORG_Name = frmData.RetData.Rows[0]["Warehouse_Name"].ToString();
            }
        }
