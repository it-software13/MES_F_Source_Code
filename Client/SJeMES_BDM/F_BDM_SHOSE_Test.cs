...
{
    public partial class F_BDM_SHOSE_Test : MaterialForm
    {
        public List<code_value_OBJ> JudgeList = new List<code_value_OBJ>(); 
        private readonly MaterialSkinManager materialSkinManager;
        public F_BDM_SHOSE_Test()
        {
            InitializeComponent();
        ...
        {
            F_BDM_SHOSE_Test_Edit frm = new F_BDM_SHOSE_Test_Edit(null);
        ...
        private void F_BDM_SHOSE_Test_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        dgvr.Cells["is_lcll_text"].Value = string.IsNullOrEmpty(dr["ISLCLL_ITEM"].ToString()) ? "" : (dr["ISLCLL_ITEM"].ToString() == "0" ? "no" : "yes");//否//是
                        dgvr.Cells["is_lcll"].Value = dr["ISLCLL_ITEM"].ToString();

                        i++;
                    }
                    GenClass.AutoSizeColumn(dgv_data);
                }
        ...
        private void btn_update_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow item in dgv_data.Rows)
            {
                if(Convert.ToBoolean(item.Cells["select"].Value))
                {
                    ...
                    editInfo.Add("is_lcll", item.Cells["is_lcll"].Value.ToString());
                    break;
                }
            }
            ...
            F_BDM_SHOSE_Test_Edit frm = new F_BDM_SHOSE_Test_Edit(editInfo);
            frm.StartPosition = FormStartPosition.CenterScreen;
            frm.ShowDialog();
            if (frm.flag)
            {
        ...
        {
            using (F_BDM_SHOSE_Test_Custom f=new F_BDM_SHOSE_Test_Custom())
            {
                f.ShowDialog();
            }
        }
