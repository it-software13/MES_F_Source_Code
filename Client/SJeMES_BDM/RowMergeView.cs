...
public partial class RowMergeView : DataGridView
{
    ...
    protected override void OnCellPainting(DataGridViewCellPaintingEventArgs e)
    {
        try
        {
            ...
            if (e.RowIndex > -1 && e.ColumnIndex > -1&&e.RowIndex<2)
            {
                DrawCell(e);
            }
            else
    ...
    private void DrawCell(DataGridViewCellPaintingEventArgs e)
    {
        ...
        foreach (string col in this.MergeColumnNames)
        {
            if (col.Contains(this.Columns[e.ColumnIndex].Name) && col.Contains(","))
            {
                blnHasData = true;
                break;
            }
        ...
        if (blnHasData && e.RowIndex != -1)
        {
            cellwidth = e.CellBounds.Width;
            Pen gridLinePen = new Pen(gridBrush);

            string colArr = string.Empty;
            foreach (string col in this.MergeColumnNames)
            {
                if (col.Contains(this.Columns[e.ColumnIndex].Name))
                {
                    colArr = col;
                    break;
                }
            ...
            StringBuilder sbdColArr = new StringBuilder(50);
            string[] tmpArr = colArr.Split(',');
            foreach (var col in tmpArr)
            {
                sbdColArr.Append(this.Rows[e.RowIndex].Cells[col].Value);
            }
            ...
            //new
            curValue = sbdColArr.ToString();
            sbdColArr.Clear();

            if (!string.IsNullOrEmpty(curValue))
            {
                #region 获取下面的行数
                for (int i = e.RowIndex; i < 2; i++)
                {
                    sbdColArr = new StringBuilder(50);
                    tmpArr = colArr.Split(',');
                    foreach (var col in tmpArr)
                    {
                        sbdColArr.Append(this.Rows[i].Cells[col].Value);
                    }
                    string valueDown = sbdColArr.ToString();
                    sbdColArr.Clear();

                    if (valueDown.Equals(curValue))
                    {
                ...
                for (int i = e.RowIndex; i >= 0; i--)
                {
                    ...
                    string valueUp = sbdColArr.ToString();
                    sbdColArr.Clear();

                    if (valueUp.Equals(curValue))
                    {
                        //this.Rows[i].Cells[e.ColumnIndex].Selected = this.Rows[e.RowIndex].Cells[e.ColumnIndex].Selected;
                        UpRows++;
                        if (e.RowIndex != i)
