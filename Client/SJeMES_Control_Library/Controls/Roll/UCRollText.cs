...
namespace SJeMES_Control_Library.Controls
{
    ...
    public class UCRollText : UserControl
    {
        ...
        void m_timer_Tick(object sender, EventArgs e)
        {
            if (rectText == Rectangle.Empty)
                return;
            if (_RollStyle == SJeMES_Control_Library.Controls.RollStyle.BackAndForth && rectText.Width >= this.Width)
            {
                return;
            }
            rectText.X = rectText.X + _moveStep * m_intdirection;
            if (_RollStyle == SJeMES_Control_Library.Controls.RollStyle.BackAndForth)
            {
                if (rectText.X <= 0)
                {
                    m_intdirection = 1;
            ...
            else if (_RollStyle == SJeMES_Control_Library.Controls.RollStyle.LeftToRight)
            {
        ...
        {
            if (_RollStyle == SJeMES_Control_Library.Controls.RollStyle.LeftToRight)
            {
                m_intdirection = 1;
                if (rectText != Rectangle.Empty)
                {
            ...
            else if (_RollStyle == SJeMES_Control_Library.Controls.RollStyle.RightToLeft)
            {
                m_intdirection = -1;
                if (rectText != Rectangle.Empty)
                {
