...
namespace SJeMES_Control_Library
{
    public partial class FrmSelectImage : Form
    {
        ...
        {
            string[] nameList = System.Enum.GetNames(typeof(SJeMES_Control_Library.FontIcons));
            var lst = nameList.ToList();
            lst.Sort();
            foreach (var item in lst)
            {
                SJeMES_Control_Library.FontIcons icon = (SJeMES_Control_Library.FontIcons)Enum.Parse(typeof(SJeMES_Control_Library.FontIcons), item);
                Label lbl = new Label();
                lbl.AutoSize = false;
                ...
                lbl.Text = "       " + item;
                lbl.Image = SJeMES_Control_Library.FontImages.GetImage(icon, 32, Color.FromArgb(255, 77, 59));
        ...
        void lbl_DoubleClick(object sender, EventArgs e)
        {
            ...
            SJeMES_Control_Library.FontIcons icon = (SJeMES_Control_Library.FontIcons)lbl.Tag;
            int intSize = ucTextBoxEx1.InputText.ToInt();
            if (intSize <= 0)
                intSize = 32;
            SelectImage = SJeMES_Control_Library.FontImages.GetImage(icon, intSize, txtForeColor.BackColor, txtBackcolor.BackColor == Color.White ? Color.Empty : txtBackcolor.BackColor);
