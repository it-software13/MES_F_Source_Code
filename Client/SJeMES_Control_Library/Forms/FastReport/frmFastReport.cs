...
namespace SJeMES_Control_Library.Forms
{
    public partial class frmFastReport : Form
    {
        ...
        public frmFastReport(string reportCode, SJeMES_Framework.Class.OrgClass Org, string WebServiceUrl)
        {
            this.Client = new SJeMES_Framework.Class.ClientClass();
            Client.WebServiceUrl = WebServiceUrl;
            Client.Org = Org;
            Dictionary<string, string> p = new Dictionary<string, string>();
            p.Add("sql", "SELECT ReportPath,ReportXML,ReportString FROM[SJEMSSYS].[dbo].[SYSREPORT01M] where ReportCode = '" + reportCode + "'");
            string XML = SJeMES_Framework.Common.WebServiceHelper.RunService(Client.WebServiceUrl, "SJ_MESAPI", "SJ_MESAPI.DataBase", "GetDataTable", p);
        ...
        public frmFastReport(string reportCode, SJeMES_Framework.Class.OrgClass Org, string WebServiceUrl, string sqlWhere)
        {
            this.Client = new SJeMES_Framework.Class.ClientClass();
            Client.WebServiceUrl = WebServiceUrl;
            Client.Org = Org;
        ...
        /// <param name="fileName">报表文件路径</param>
        /// <param name="fastReportXml">报表条件XML</param>
        public frmFastReport(SJeMES_Framework.Class.ClientClass Client, string fileName, string fastReportXml)
        {
            this.Client = Client;
            this.fileName = fileName;
            this.fastReportXml = fastReportXml;
            this.connectionString = @"Data Source=" + Client.Org.DBServer + @";AttachDbFilename=;Initial Catalog=" + Client.Org.DBName + @";
        ...
        /// <param name="fastReportXml">报表条件XML</param>
        /// <param name="dicParameter">参数键值对</param>
        public frmFastReport(SJeMES_Framework.Class.ClientClass Client, string fileName, string fastReportXml, Dictionary<string, string> dicParameter)
        {
            this.Client = Client;
            this.fileName = fileName;
            this.fastReportXml = fastReportXml;
        ...
        public frmFastReport(SJeMES_Framework.Class.OrgClass Org, string WebServiceUrl, Dictionary<string, string> dic)
        {
            try
            {
        ...
        public frmFastReport(SJeMES_Framework.Class.OrgClass Org, Dictionary<string, string> dic, string WebServiceUrl)
        {
            try
            {
                string reportCode = (dic as Dictionary<string, string>)["ReportCode"];
                            ...
                            if (lstDataSources.Count > 1)//当有多个数据源的时候，表头没数据，表身也不要显示数据
                            {
                                ...
                                foreach (var item in lstDataSources)
                                {
                                    ...
                                    if (n == 1)
                                    {
                                        if (Client != null)
                                        {
                                            tab= Client.GetDataTable("select top 1*  from(" + searchSQL + ")tab where 1=1 " + newSQLWhere);
                                        }
                                        else
                                        {
                                            Dictionary<string, string> p = new Dictionary<string, string>();
                                            ...
                                            string XML = SJeMES_Framework.Common.WebServiceHelper.RunService(Client.Org, Client.WebServiceUrl, "SJ_MESAPI", "SJ_MESAPI.DataBase", "GetDataTable", p);
                                            string dtXML = SJeMES_Framework.Common.StringHelper.GetDataFromFirstTag(XML, "<RetData>", "</RetData>");
        ...
        private void Txt_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Client != null)
                {
                    FrmSelectData frm = new FrmSelectData(txt.Tag.ToString(), true, Client);
                    frm.ShowDialog();
                    if (frm.RetData.Rows.Count>0)
                    {
                        txt.Text = frm.RetData.Rows[0][txt.Name].ToString();
                    }
                }
