...
namespace SJeMES_Control_Library.Forms
{
    public partial class frmReportSetting : Form
    {
        ...
        public frmReportSetting(SJeMES_Framework.Class.ClientClass Client, string reportCode, string moduleNo)
        {
            this.Client = Client;
            this.reportCode = reportCode;
            this.moduleNo = moduleNo;
        ...
        {
            FrmAddControl frm = new FrmAddControl(groupBox2, Client);
            frm.ShowDialog();
        ...
        private void Dtp_MouseUp(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                FrmAddControl frm = new FrmAddControl(dtp,groupBox2, Client);
                frm.ShowDialog();
                if (frm.ctrType == "文本框")
                {
        ...
        private void Txt_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                FrmAddControl frm = new FrmAddControl(txt, groupBox2, Client);
                frm.ShowDialog();
        ...
        private void cbo_MouseClick(object sender, MouseEventArgs e)
        {
            try
            {
                ...
                FrmAddControl frm = new FrmAddControl(cbo, groupBox2, Client);
                frm.ShowDialog();
                if (frm.ctrType == "文本框")
                {
                    TextBox textBox = new TextBox();
                                        ...
                                        if (cc is Panel)
                                        {
                                            ...
                                            foreach (Control t in p.Controls)
                                            {
                                                ...
                                                if (t is TextBox)
                                                {
                                                    if (Client != null)
                                                    {
                                                        tabData = Client.GetDataTable(t.Text);
                                                    }
                                                    else
                                                    {
                                                        Dictionary<string, string> pp = new Dictionary<string, string>();
                                                        pp.Add("sql", t.Text);
                                                        string XML = SJeMES_Framework.Common.WebServiceHelper.RunService(Client.Org, Client.WebServiceUrl, "SJ_MESAPI", "SJ_MESAPI.DataBase", "GetDataTable", pp);
                                                        string dtXML = SJeMES_Framework.Common.StringHelper.GetDataFromFirstTag(XML, "<RetData>", "</RetData>");
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(reportCode))
                {
                    ...
                    if (Client != null)
                    {
                        tabData = Client.GetDataTable(sql);

                    }else
                    {
                        Dictionary<string, string> pp = new Dictionary<string, string>();
                        pp.Add("sql", sql);
                        string XML = SJeMES_Framework.Common.WebServiceHelper.RunService(Client.WebServiceUrl, "SJ_MESAPI", "SJ_MESAPI.DataBase", "GetDataTable", pp);
                        string dtXML = SJeMES_Framework.Common.StringHelper.GetDataFromFirstTag(XML, "<RetData>", "</RetData>");
                        tabData = SJeMES_Framework.Common.StringHelper.GetDataTableFromXML(dtXML);
                    }
                    ...
                    if (Client != null)
                    {
                       int n= Client.ExecuteNonQuery(sql);
                        if (n > 0)
                            b = true;
                    }else
                    {
                        Dictionary<string, string> pp = new Dictionary<string, string>();
                        pp.Add("sql", sql);
                        string XML = SJeMES_Framework.Common.WebServiceHelper.RunService(Client.WebServiceUrl, "SJ_MESAPI", "SJ_MESAPI.DataBase", "ExecuteNonQuery", pp);
        ...
        private void btn_Pre_Click(object sender, EventArgs e)
        {
            ...
            frmFastReport frm = new frmFastReport(reportPath, xml, connectionString, reportString);
            frm.Show();
        }
