...
namespace SJeMES_Control_Library.Forms
{
    /// <summary>
    /// Class FrmInputs.
    /// Implements the <see cref="SJeMES_Control_Library.Forms.FrmWithOKCancel1" />
    /// </summary>
    /// <seealso cref="SJeMES_Control_Library.Forms.FrmWithOKCancel1" />
    public partial class FrmInputs : FrmWithOKCancel1
    {
        ...
        /// <param name="ReadOnlyLabels">只读的项</param>
        /// <param name="fromwidth">窗体宽度</param>
        ...
        public FrmInputs(
            ...
            Dictionary<string, SJeMES_Control_Library.Controls.KeyBoardType> keyBoards = null,
            List<string> mastInputs = null,
            Dictionary<string, string> defaultValues = null,
            List<string> ReadOnlyLabels = null,
            int fromwidth =500)
        {
            InitializeComponent();
            this.Title = strTitle;
            this.Width = fromwidth; 
            this.btnOK.BtnText = "确定(OK)";
            this.btnCancel.BtnText = "取消(Cancel)";
            ...
            try
            {
                ...
                SJeMES_Control_Library.ControlHelper.FreezeControl(this, true);
                ...
                for (int i = inPutLabels.Length - 1; i >= 0; i--)
                {
                    ...
                    SJeMES_Control_Library.Controls.UCTextBoxEx txt = new Controls.UCTextBoxEx();
                    txt.Dock = DockStyle.Fill;
                    txt.IsShowKeyboard = false;
                    txt.IsShowClearBtn = false;
                    txt.Name = "txt_" + i;
                    txt.TabIndex = i;
                    if(ReadOnlyLabels !=null && ReadOnlyLabels.Contains(inPutLabels[i]))
                    {
                        txt.txtInput.ReadOnly = true;
                    }
            ...
            {
                SJeMES_Control_Library.ControlHelper.FreezeControl(this, false);
            }
        }
        #endregion
        ...
        protected override void DoEnter()
        {
            for (int i = 0; i < Values.Length; i++)
            {
                ...
                if (cs.Length > 0)
                {
                    var txt = cs[0] as SJeMES_Control_Library.Controls.UCTextBoxEx;
                    Values[i] = txt.InputText;
                    if (m_mastInputs.ContainsKey(i) && string.IsNullOrWhiteSpace(txt.InputText))
                    {
                        SJeMES_Control_Library.Forms.FrmTips.ShowTipsInfo(this, "[" + m_mastInputs[i] + "]必须输入。");                        
