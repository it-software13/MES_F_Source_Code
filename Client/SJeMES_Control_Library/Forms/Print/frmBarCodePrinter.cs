...
namespace SJeMES_Control_Library.Forms
{
    public partial class frmBarCodePrinter : Form
    {
        ...
        public frmBarCodePrinter(SJeMES_Framework.Class.ClientClass Client, string WebServiceUrl, string sqlSelectData)
        {
            InitializeComponent();
        ...
        public frmBarCodePrinter(SJeMES_Framework.Class.ClientClass Client, string WebServiceUrl, DataTable dt, string printType)
        {
            InitializeComponent();
            this.dataTable = dt;
            this.WebServiceUrl = WebServiceUrl;
            ...
            this.Client = Client;
            //          comboBox1.SelectedIndex = 0;
            //          DataTable vds = new DataTable();//空白
            //           vds = dt.Clone();
            //           //limit 显示条数 page 页码数
            //int fromIndex =int.Parse(comboBox1.Text) * (pageNo - 1);//开始行
            //           int toIndex = int.Parse(comboBox1.Text) * pageNo - 1; //结束行
            //          for (int i = fromIndex; i <= toIndex; i++)
            //               {
            //                   if (i >= (dt.Rows.Count)) //到达最后一行，退出
            //                           break;
        ...
        public frmBarCodePrinter(SJeMES_Framework.Class.ClientClass Client, string WebServiceUrl, List<string> BarCodes)
        {
            InitializeComponent();
        ...
        public frmBarCodePrinter(SJeMES_Framework.Class.ClientClass Client, string WebServiceUrl, DataTable dt)
        {
            InitializeComponent();
            this.dataTable = dt;
            this.WebServiceUrl = WebServiceUrl;
        ...
        public frmBarCodePrinter(SJeMES_Framework.Class.ClientClass Client, string WebServiceUrl, string sqlSelectData, string printType, DataTable dt)
        {
            InitializeComponent();
            this.dataTable = dt;
            this.sqlSelectData = sqlSelectData;
        ...
        private void frmBarCodePrinter_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                List<SJeMES_Framework.Common.IOHelper.File> Files = new List<SJeMES_Framework.Common.IOHelper.File>();
                Files = SJeMES_Framework.Common.IOHelper.GetAllFile(Application.StartupPath + @"\Printer\BarCodeModel", Files, 1);
                ...
                if (string.IsNullOrEmpty(sqlSelectData))
                {
                    ...
                    foreach (SJeMES_Framework.Common.IOHelper.File f in Files)
                    {
        ...
        private void BtnModel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string sql = SJeMES_Framework.Common.TXTHelper.ReadToEnd(Application.StartupPath + @"\Printer\BarCodeModel\" + ((Button)sender).Name + ".sql");
...
" + where + where1 + @" order by 条码";
        ...
        private string GetBarCode()
        {
            ...
            try
            {
                FrmSelectData frm = new FrmSelectData(sqlSelectData, true, Client);
                frm.ShowDialog();
                ...
                {
                    ColName = frm.RetData.Columns[0].ColumnName;
                    ret = frm.RetData.Rows[0][0].ToString();
                }
            }
            catch (Exception ex)
            {
        ...
        private void GetBarCodes()
        {
            ...
            try
            {
                ...
                sql = "select manager_no from HR002A1 WHERE staff_no='" + Client.Org.User.UserCode + "'";
                var tab = SJeMES_Framework.Common.WebServiceHelper.GetDataTable(Client.Org, Client.WebServiceUrl, sql, new Dictionary<string, string>());
                ...
                if (!string.IsNullOrEmpty(manager_no))
                {
                    ...
                    tab = SJeMES_Framework.Common.WebServiceHelper.GetDataTable(Client.Org, Client.WebServiceUrl, sql, new Dictionary<string, string>());
                    for (int i = 0; i < tab.Rows.Count; i++)
                    {
                ...
                if (!string.IsNullOrEmpty(txt_BarCodeS.Text.Trim()))
                {
                    sql = @"
SELECT
count(*) as '行数'
...
(" + newSql + @") TMP";
                ...
                pageCount = (int)Math.Ceiling(Convert.ToDecimal(Client.GetDataTable(sql).Rows[0][0].ToString()) / Convert.ToDecimal(comboBox1.Text));
...
FROM
(" + newSql + @") TMP
)tab  where tab.序号>" + (pageNo - 1) * Convert.ToInt32(comboBox1.Text);

                }

                tab = Client.GetDT(sql);
                dataGridView1.DataSource = tab;
                ...
                //string xml = Common.WebServiceHelper.RunService(Org, WebServiceUrl, "SJEMS_API", "SJEMS_API.DataBase", "GetDataTable", p);
