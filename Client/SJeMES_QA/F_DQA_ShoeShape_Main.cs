...
namespace SJeMES_QA
{
    public partial class F_DQA_ShoeShape_Main : MaterialForm
    {
        private readonly MaterialSkinManager materialSkinManager;
        public F_DQA_ShoeShape_Main()
        {
            InitializeComponent();
...
Program.SkinThemes, materialSkinManager, this);
        ...
        public void GetDQAMain(int pageSize, int pageIndex, out int totalCount)
        {
            totalCount = 0;
            try
            {
                ...
                {
                    string msg = SJeMES_Framework.Common.UIHelper.UImsg("Please enter any one search condition！", Program.Client, Program.Client.WebServiceUrl, Program.Client.Language);//请添加条件再搜索
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        //dgvr.Cells["鞋图"].Value = dr["DEVELOP_SEASON"].ToString();//鞋图
                        dgvr.Cells["xt_url"].Value = dr["file_url"].ToString();
                ...
                this.dataGridView1.Columns["开发品质记录"].DefaultCellStyle.SelectionBackColor = Color.Transparent;
        ...
        private void F_DQA_ShoeShape_Main_Load(object sender, EventArgs e)
        {
            ...
            pageControl1.BindPageEvent += GetDQAMain;
            //LoadPage();
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (this.dataGridView1.Columns[e.ColumnIndex].Name == "开发品质记录")
                {
                    DataGridViewOperationCell cell = this.dataGridView1.Rows[this.dataGridView1.CurrentRow.Index].Cells["开发品质记录"] as DataGridViewOperationCell;
                    if (cell.CurrentItem == null)
                    {
                        return;
                    }
                    else if (cell.CurrentItem.Equals("阶段样品记录"))//阶段样品记录
                    {
                        string shoe_no = dataGridView1.Rows[e.RowIndex].Cells["SHOE_NO"].Value.ToString();
                        string develop_season = dataGridView1.Rows[e.RowIndex].Cells["develop_season"].Value.ToString();
                        string user_section = dataGridView1.Rows[e.RowIndex].Cells["user_section"].Value.ToString();
                        string frmName = $@"F_DQA_ShoeShape_trait_Main_{shoe_no}";
                        var findFrm = Application.OpenForms[frmName];
                        if (findFrm == null)
                        {
                            F_DQA_ShoeShape_trait_Main update = new F_DQA_ShoeShape_trait_Main(shoe_no, develop_season, user_section);
                            update.Name = frmName;
                            update.Show();
                        }
                        else
                ...
                if (this.dataGridView1.Columns[e.ColumnIndex].Name == "QA文件管理")
                {
                    ...
                    else if (cell.CurrentItem.Equals("查看详情"))//查看详情
                    {
                        ...
                        string frmName = $@"F_DQA_ShoeShape_List_{shoe_no}";
                        var findFrm = Application.OpenForms[frmName];
                        if (findFrm == null)
                        {
                            F_DQA_ShoeShape_List update = new F_DQA_ShoeShape_List(shoe_no, user_fdd);
                            update.Name = frmName;
                            update.Show();
                        }
                        else
                    ...
                    else if (cell.CurrentItem.Equals("DQA管理"))//DQA管理
                    {
                        string user_fdd = dataGridView1.Rows[e.RowIndex].Cells["user_fdd"].Value.ToString();
                        string shoe_no = dataGridView1.Rows[e.RowIndex].Cells["SHOE_NO"].Value.ToString();
                        string frmName = $@"F_DQA_ShoeShape_Edit_{shoe_no}";
                        var findFrm = Application.OpenForms[frmName];
                        if (findFrm == null)
                        {
                            F_DQA_ShoeShape_Edit update = new F_DQA_ShoeShape_Edit(shoe_no, this, user_fdd);
                            update.Name = frmName;
                            update.Show();
                        }
                        else
