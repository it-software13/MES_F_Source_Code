...
namespace SJeMES_QA
{
    public partial class F_DQA_ShoeShape_List : MaterialForm
    {
        private readonly MaterialSkinManager materialSkinManager;
        string shoe_no = string.Empty;
        string user_fdd = string.Empty;

        public F_DQA_ShoeShape_List()
        {
            InitializeComponent();
            aa();
...
Program.SkinThemes, materialSkinManager, this);
        ...
        public F_DQA_ShoeShape_List(string _shoe_no,string _user_fdd)
        {
            shoe_no = _shoe_no;
            user_fdd = _user_fdd;
            InitializeComponent();
...
Program.SkinThemes, materialSkinManager, this);
        ...
        private void F_DQA_ShoeShape_List_Load(object sender, EventArgs e)
        {
        ...
        public void GetShoeShape_Edit(string shoe_no)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    //label16.Text = dt.Rows[0]["USER_IN_SHOECHARGE"].ToString();
                    ...
                    textBox1.Text = dt.Rows[0]["qa_principal"].ToString();
        ...
        /// <returns></returns>
        public DataTable GetShoeShape_ListFile()
        {
        ...
        {
            FrmFileList add = new FrmFileList(GetShoeShape_ListFile(), Program.Client.UploadUrl, Program.Client.UserToken);
            add.Show();
        }
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        dgvr.Cells["imageguids"].Value = dr["imageguids"].ToString();//所有图片guid
                            ...
                            try
                            {
                                ...
                                foreach (var imgInfo in imgsList)
                                {
                                    ...
                                    {//是主图
                                        ...
                                        Image image = new Bitmap(webC.OpenRead(url));
                                        dgvr.Cells["image_guid"].Value = image;
                                        break;
                                    }
                                }
                            }
                        ...
                        {
                            dgvr.Cells["image_guid"].Value = null;
                        }
                        ...
                        dgvr.Cells["Visualinspection"].Value = dr["inspection_type"].ToString();
                        ...
                        dgvr.Cells["file_id"].Value = dr["file_id"].ToString();
        ...
        /// <summary>
        /// GetShoeShape_ListFileguid
        /// </summary>
        /// <param name="fjguid"></param>
        /// <returns></returns>
        public DataTable GetShoeShape_ListFileguid(string fjguid)
        {
            DataTable dt = new DataTable();
            try
            {
                ...
                Dictionary<string, object> data = new Dictionary<string, object>();
                //键值对传值
                data.Add("file_id", fjguid);//guid
                string retdata = SJeMES_Framework.WebAPI.WebAPIHelper.Post(
                                            Program.Client.APIURL,
                                            "SJ_BDMAPI",//类库名
                                            "SJ_BDMAPI.DQA_ShoeShape",//类名
                                            "GetShoeShape_ListFileguid",//方法名
                                            Program.Client.UserToken,//token
                                            Newtonsoft.Json.JsonConvert.SerializeObject(data));
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex > -1)
            {
                if (dataGridView1.Columns[e.ColumnIndex].Name == "accessory")
                {
                    ...
                    if (cell.CurrentItem.Equals("accessorylist"))
                    {
                        string fjguid = dataGridView1.Rows[e.RowIndex].Cells["file_id"].Value.ToString();
                        FrmFileList add = new FrmFileList(GetShoeShape_ListFileguid(fjguid), Program.Client.UploadUrl, Program.Client.UserToken);
                        add.Show();
                    }
                }
                //查看图片
                if (dataGridView1.Columns[e.ColumnIndex].Name == "operation")
                {
                    ...
                    {
                        try { dataGridView1.Rows[e.RowIndex].Cells["tp"].Value = null; } catch (Exception ex) { }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                HeadDic.Add("QA负责人", textBox1.Text);
                ...
                dts.Columns.Remove("inspection_type");
                dts.Columns.Remove("inspection_code");

                ExeclHelper.ExportToTrueExcelEx(dts, HeadDic, Execldic, "DQA_View_List");//DQA查看列表
            }
            catch (Exception ex)
            {
                string msg = SJeMES_Framework.Common.UIHelper.UImsg(ex.Message, Program.Client, Program.Client.WebServiceUrl, Program.Client.Language);
